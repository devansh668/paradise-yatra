...
const getAllPackages = async (req, res) => {
    try {
        ...
        console.time('findPackages');
        let packagesQuery = AllPackage.find(query)
            .sort({ createdAt: -1 })
            .limit(parseInt(limit))
            .skip((parseInt(page) - 1) * parseInt(limit));
        
        if (minimal) {
            packagesQuery = packagesQuery.select('name slug state country image tourType isActive');
        }
        
        let packages = await packagesQuery;
        console.timeEnd('findPackages');
