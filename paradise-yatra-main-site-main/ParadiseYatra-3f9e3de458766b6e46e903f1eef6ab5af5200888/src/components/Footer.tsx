...
const Footer = () => {
  ...
  useEffect(() => {
    ...
    const fetchLocations = async () => {
      try {
        const indiaResponse = await fetch("/api/all-packages?tourType=india&limit=200&isActive=true&minimal=true", { cache: "no-store" });
        if (indiaResponse.ok) {
        ...
        const intlResponse = await fetch("/api/all-packages?tourType=international&limit=200&isActive=true&minimal=true", { cache: "no-store" });
        if (intlResponse.ok) {
